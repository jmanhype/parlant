Feature: Supervision
    ...
    Scenario: Prioritizing freezing transactions over processing refunds
        ...
        And that the "freeze_transactions" guideline is proposed with  has a priority of 10 because "An error in the last transaction occurred, which is unusual and raises concerns."
        And that the "process_refund" guideline is proposed with  has a priority of 9 because "The customer explicitly requested a refund."
    ...
    Scenario: Prioritizing transferring the upset customer to the manager over offering pizza
        ...
        And that the "selling_pizza" guideline is proposed with  has a priority of 7 because "The conversation was initiated, indicating a new customer has arrived."
        And that the "customer_upset" guideline is proposed with  has a priority of 8 because "The customer is visibly upset about the wait."
        When messages are produced
